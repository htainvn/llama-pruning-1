...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', type=str, default='What is the capital of France?', help='Prompt to generate the output.')
    ...
    output = get_output(args.prompt, model, tokenizer, device=device, apply_chat_template=args.apply_chat_template, max_new_tokens=args.max_new_tokens)
    ...
    output_pruned = get_output(args.prompt, model, tokenizer, device=device, apply_chat_template=args.apply_chat_template, max_new_tokens=args.max_new_tokens)

    print(f'Saving the pruned model at {args.output}...')
